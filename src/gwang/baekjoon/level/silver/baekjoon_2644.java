...
public class baekjoon_2644 {
	 ...
	 * 나와 아버지 형제들과는 3촌이 된다.여러 사람들에 대한 부모 자식들 간의 관계가 주어졌을 때, 
	...
	public static void main(String[] args) throws IOException {
		...
		x = Integer.parseInt(arr[0]); y = Integer.parseInt(arr[1]);
		...
		for(int i=0; i<m; i++) {
			String[] relArr = br.readLine().split(" ");
			int parent = Integer.parseInt(relArr[0]); 
			int child = Integer.parseInt(relArr[1]);
			
			if(relations[parent] == null) {
				Relation rel = new Relation();
				rel.me = parent;
				rel.children.add(child);
				relations[parent] = rel;
			} else {
				Relation tmpRel = relations[parent];
				tmpRel.children.add(child);
				relations[parent] = tmpRel;
			}
			
			if(relations[child] == null) {
				Relation rel = new Relation();
				rel.me = child;
				rel.parent = parent;
				relations[child] = rel;
	...
	public static void bfs(int start, int target) {
		...
		Relation first = relations[start];
		queue.add(first);
		visited[start] = true;
		...
		while(!queue.isEmpty()) {
			...
			// parent
			if(!visited[now.parent] && now.parent != 0) {
				Relation next = relations[now.parent];
				next.chon = now.chon + 1;
				queue.add(next);
			...
			if(now.children.size() > 0) {
				for(int i=0; i<now.children.size(); i++) {
					int childId = now.children.get(i);
					if(!visited[childId]) {
						Relation child = relations[childId];
						child.chon = now.chon+1;
						queue.add(child);
